...
export async function GET() {
  ...
  if (!key) return NextResponse.json({ error: "Realtime voice not configured" }, { status: 503 });
  ...
  if (!r.ok || !token) return NextResponse.json({ error: "Could not mint a realtime token: " + (j.error ?? r.status) }, { status: 502 });
