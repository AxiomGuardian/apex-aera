...
import * as cloud from "@/lib/chat/cloudSync";
...
export function AERAProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (data.threads.length) {
        ...
        const active = data.threads.find((t) => t.id === activeThreadRef.current) ?? data.threads[0];
        activeThreadRef.current = active.id;
  ...
  const addUserMessage = useCallback(async (content: string) => {
    ...
    void cloud.saveMessages(activeThreadRef.current, [userMsg]);
    setIsTyping(true);
    ...
    try {
      ...
      setThreads([...threadsRef.current]);
      void cloud.saveMessages(activeThreadRef.current, newMessages);
      ...
      setThreads([...threadsRef.current]);
      void cloud.saveMessages(activeThreadRef.current, [fallback]);
      if (voiceModeRef.current) {
        const agentVoiceId = AGENTS[selectedAgentIdRef.current]?.voice_id;
  ...
  const clearHistory = useCallback(() => {
    stopSpeaking();
    void cloud.clearThreadMessages(activeThreadRef.current);
  ...
  const createThread = useCallback((name?: string, folderId: string | null = null) => {
    ...
    void cloud.upsertThread(newThread);
  ...
  const renameThread = useCallback((id: string, name: string) => {
    ...
    void cloud.renameThread(id, name);
    setThreads([...threadsRef.current]);
    saveThreads(threadsRef.current, foldersRef.current, activeThreadRef.current);
  }, []);
  ...
  const moveThread = useCallback((threadId: string, folderId: string | null) => {
    ...
    void cloud.moveThread(threadId, folderId);
    setThreads([...threadsRef.current]);
    saveThreads(threadsRef.current, foldersRef.current, activeThreadRef.current);
  }, []);
  ...
  const createFolder = useCallback((name: string) => {
    ...
    void cloud.upsertFolder(folder);
  ...
  const renameFolder = useCallback((id: string, name: string) => {
    ...
    void cloud.renameFolder(id, name);
    setFolders([...foldersRef.current]);
    saveThreads(threadsRef.current, foldersRef.current, activeThreadRef.current);
  }, []);
